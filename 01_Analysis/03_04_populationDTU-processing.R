...
## @RCG 2025-03-06 This script performs the initial processing of the population DTU output.
...
##  2. Load data ------------------------------------------
file.l <- list("lineage" = read.RDS.files(path = "../01_popDTU/06_celltype_fractions/01_lineage/", pattern = "popDTU.rds"))#, 
               #"celltype" = read.RDS.files(path = "../01_popDTU/06_celltype-fractions/02_celltype/", pattern = "popDTU.rds"))

##  2. Process ------------------------------------------

###  2.1 Select coef of interest ------------------------------------------
dtu.l <- select.df.from.nested.list(file.l, "POP")

###  2.2 Annotate --> Annotated upstream of MANTA
# tx2gene <- read.table("../../../../../01_Quantif/00_IndexData/02_GENCODEv42_filt-IAV-SARS_Cov_2-tx2gene-with-ENS.tsv", header = T, sep = "\t", na.strings = "")
# # Add gene_name # Already added
# dtu.l <- add.gene_name.2.gene_id(df = dtu.l, tx2gene, gene_id.col = "gene_id", gene_name.col = "gene_name")
# # Add tx_names
# dtu.l <- add.tx_name.2.tcc_id(df = dtu.l, tx2gene, tcc_id.col = "feature_ids", tx_name.col = "transcript_name")

###  2.3 Add DGE info ------------------------------------------
dge.l <- readRDS("../../../../01_DGE/02_popDGE/02_DA/01_Processed-Data/05_include-Library/01_popDGE-filtered-list.rds")
# dge.l <- dge.l["lineage"]
...
add.dge.info <- function(dtu, dge){
  if( class(dtu) == "list" & class(dge) == "list" ){ mapply(add.dge.info, dtu, dge, SIMPLIFY = F) # if list, iterate
    ...
    # directionality
    dtu[["DGE.direction"]] <- ifelse( dge[ match(dtu$gene_id, dge$genes), "betas"]  > 0 , yes = "Up", no = "Down" ) # we do betas > 0 because DGE already filtered
    return(dtu)
  }
}
dtu.l <- add.dge.info(dtu.l["lineage"], dge.l["lineage"])
...
file.name <- "01_Processed-Data/06_celltype-fractions/01_popDTU-unfiltered-df.rds"
saveRDS(dtu.l, file.name)
...
print("Processing of popDTU results finished !")
